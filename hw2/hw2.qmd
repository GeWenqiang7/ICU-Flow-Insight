...
labevents_parquet_ds <- open_dataset(
  path = "~/biostat-203b-2025-winter/hw2/labevents_parquet", 
  format = "parquet"
)

# Select only required columns and filter for specific itemid values
labevents_filtered_parquet <- labevents_parquet_ds %>%
  select(subject_id, itemid, charttime, valuenum) %>%
  filter(itemid %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) %>%
...
cat("Time taken for ingestion, selection, and filtering (Parquet):", round(elapsed_time, 2), "seconds\n")

# Display number of rows in the filtered dataset
cat("Number of rows in filtered dataset (Parquet):", nrow(labevents_filtered_parquet), "\n")
