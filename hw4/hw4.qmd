...
mimic_icu_cohort <- mimic_icu_cohort |>
  mutate(
    race = fct_collapse(race,  # Collapse race categories
      ASIAN = c("ASIAN", "ASIAN - CHINESE", 
                "ASIAN - JAPANESE", "ASIAN - INDIAN"),
      BLACK = c("BLACK", "BLACK/AFRICAN AMERICAN"),
      HISPANIC = c("HISPANIC", "HISPANIC/LATINO"),
      WHITE = c("WHITE", "WHITE - RUSSIAN"),
      Other = c("OTHER", "UNKNOWN", "MIXED", 
...
mimic_icu_cohort <- mimic_icu_cohort |>
  mutate(
    los_long = los >= 2  # TRUE if length of stay is 2 days or more, FALSE otherwise
  )
```
