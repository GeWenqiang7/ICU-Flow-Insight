...
rm(patients,admissions,transfers,procedures,diagnoses)
```
...
final_plot <- ggplot() +
  ...
  # Procedure Events 
  geom_point(data = procedure_data, aes(x = chartdate, y = "Procedure", shape = long_title), 
             size = 6) +  
  ...
  geom_segment(data = adt_data, aes(x = intime, xend = outtime, y = "ADT", yend = "ADT", 
                                    color = careunit, 
                                    size = ifelse(careunit %in% c("Surgical Intensive Care Unit (SICU)", 
                                                                  "Medical Intensive Care Unit (MICU)"), 10, 5))) +
  scale_size_identity() +  

  # 调整 X 轴间距，使日期之间网格更宽
  scale_x_datetime(date_labels = "%b %d", date_breaks = "2 week", expand = c(0.1, 0.1)) +

  # 调整标题和标签
  labs(title = "Patient ADT, Lab, and Procedure History",
       x = "Calendar Time",
       y = "Event Type",
       color = "Care Unit",
       shape = "Procedure") +
  ...
  # 主题优化
  theme_minimal() +
  theme(legend.position = "bottom",  # 图例放到底部
        legend.box = "vertical",   # 让图例竖直排列
        legend.key.height = unit(0.5, 'cm'),  # 增加图例项高度，避免遮挡
        legend.spacing.y = unit(0.4, 'cm'),  # 调整 Care Unit 和 Procedure 之间的间距
        axis.text.y = element_text(size = 12),  
        axis.ticks.y = element_blank()) +
  guides(color = guide_legend(nrow = 2), shape = guide_legend(nrow = 2))  # 图例分成 2 行
...
```
```{r}
rm(admissions_data,lab_data,final_plot,transfers_data,
   procedure_data,procedures_data,d_icd_diagnoses,
   d_icd_procedures,patient_data,procedure_plot,
   adt_plot,adt_data,diagnoses_data)
