...
duckdb_table <- arrow::to_duckdb(labevents_table, 
                                 con = con, 
                                 table_name = "labevents_duckdb")

result_labevents <- tbl(con, "labevents_duckdb") %>%
  select(subject_id, itemid, storetime,itemid, valuenum) %>%
...
```{r}
# Merge `result_icustays` with `result_labevents` on `subject_id` and `stay_id`
merged_data <- result_labevents %>%
  inner_join(result_icustays, by = c("subject_id")) %>%
  filter(storetime < intime) %>%  # Ensure storetime is earlier than ICU admission
  select(subject_id, stay_id, itemid, storetime, valuenum) 

# Select the latest (`storetime`) measurement per `itemid` before ICU stay
latest_labs <- merged_data %>%
  group_by(subject_id, stay_id, itemid) %>%
  slice_max(order_by = storetime, n = 1, with_ties = FALSE) %>%  # Keep only the most recent lab result
  ungroup()
...
```{r}
# Remove `storetime` before pivoting
latest_labs1 <- latest_labs %>%
  select(-storetime)  # Remove storetime before pivot_wider()

# Step 3: Ensure there are no duplicate rows per (`subject_id, stay_id, itemid`)
latest_labs <- latest_labs1 %>%
  distinct(subject_id, stay_id, itemid, .keep_all = TRUE)

rm(latest_labs1)
```

```{r}
# Pivot `itemid` to become separate columns with the most recent `valuenum`
labevents_tble <- latest_labs %>%
  pivot_wider(names_from = itemid, values_from = valuenum)

# Rename columns for clarity
colnames(labevents_tble) <- c("subject_id", "stay_id", "bicarbonate",
                          "chloride","creatinine", "glucose",
                          "potassium", "sodium", "hematocrit", "wbc")
# group by readability
labevents_tble <- labevents_tble %>% arrange(subject_id, stay_id)

# View the final dataset
print(head(labevents_tble, 10), width = Inf)
...
dbExecute(con, "CREATE TABLE chartevents_duckdb AS 
                ...
                FROM read_parquet('~/biostat-203b-2025-winter/hw2/chartevents_parquet/part-0.parquet')
...
```{r}
# Join with ICU patients and keep only first vital measurement within ICU stay
dbExecute(con, "CREATE TABLE latest_vitals AS 
                SELECT subject_id, stay_id, itemid, storetime, valuenum
                FROM (
                    SELECT c.subject_id, i.stay_id, c.itemid, c.storetime, c.valuenum,
                           ROW_NUMBER() OVER (PARTITION BY c.subject_id, i.stay_id, c.itemid 
                                              ORDER BY c.storetime ASC) AS rn
...
dbExecute(con, "CREATE TABLE latest_vitals_filtered AS 
                ...
                FROM (
                    SELECT *, ROW_NUMBER() OVER (PARTITION BY subject_id, stay_id, itemid ORDER BY storetime ASC) AS rn
                    FROM latest_vitals
                ) WHERE rn = 1")
...
# View the final dataset
print(head(chartevents_tble, 10), width = Inf)
