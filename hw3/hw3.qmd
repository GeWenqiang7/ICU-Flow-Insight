...
ggsave("final_plot.png", final_plot, width = 15, height = 6, dpi = 300)
print(final_plot)
...
dbExecute(con, "
  ...
  SELECT c.subject_id,c.itemid,c.storetime,c.valuenum,
...
vital_data <- dbGetQuery(con, "
  SELECT subject_id, stay_id, itemid, storetime, valuenum, intime, outtime
  FROM vital  
  WHERE storetime BETWEEN intime AND outtime
")

rm(con,icustays,patient_data)
```
...
```{r}
ggplot(vital_data, aes(x = storetime, y = valuenum, color = vital_type)) +
  geom_point(size = 1.5) +
...
# **Save the plot**
ggsave("icu_vitals_plot.png", width = 12, height = 6, dpi = 300)

rm(vital_data,vital_type,itemid_labels)
